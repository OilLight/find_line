...
def get_crosser_point(points1 ,mask=True, points2=[],size_w=80):#,size_h=60):
    crossover_x = 0
    crossover_y = 0
...
def range_check(img,input_threshold,crosser_point, end_point, ratio, size_w=80, size_h=60, the_range=20):
    ...
    if input_y <0:
        input_y = 0
    if tmp_x+half_range >tmp_w:
        input_w = the_range-(tmp_x+half_range-tmp_w)
    if tmp_y+half_range >tmp_h:
        input_h = the_range-(tmp_y+half_range-tmp_h)
    blobs = img.find_blobs([(input_threshold,255)],False,(input_x,input_y,input_w,input_h),2,1,10,20,True)
...
def cal_piexl(input_img,input_threshold):
    ...
    return pixels
def abnormal_threshold_check(last_img,realtime_img,last_threshold,realtime_threshold,consult_threshold,ratio,consult_thresholds,size_w=80,size_h=60):
    ...
    if abs(sub_threshold) <= 5:
        ...
        if piexl_realtime >size_w*size_h*0.4:
...
last_list =[0,int(size_w*0.5),0]
extra_list = [0,int(size_h*0.5),0]
mask_none = False
...
while(True):
    ...
    realtime_lab_threshold = [(threshold_l,100,threshold_a,127,threshold_b,-128)]
    tmp_img = corr_img.copy()
    #corr_img.to_grayscale()
    bin_img = tmp_img.binary(realtime_lab_threshold)
    bin_img.open(1)

    #stdev = tmp_statistics.stdev()
    #print("stdev: "+str(stdev))
    #realtime_gray_threshold = tmp_histogram.get_threshold().value()
    #realtime_gray_threshold = tmp_statistics.mean()

    lines = corr_img.find_lines((0,0,size_w,size_h),2,1,600)
    ...
    if lines:
        ...
        for i in range(len(lines)):
            ...
            for j in range(len(lines)):
                ...
                if 70<=sub<=105 or 30<=sub<=50 or 120<=sub<=140:
                    ...
                    for k in range(4):
                        #closer range
                        blobs[k] = range_check(bin_img,100,crossover_point,points[k],0.33,size_w,size_h)
                        #further range
                        blobs[k+4] = range_check(bin_img,100,crossover_point,points[k],0.8,size_w,size_h)
        ...
        if not mask_none:
            ...
            for the_line in lines:
                ...
                blobs = []
                blobs_t_c = range_check(bin_img,100,crossover_point,point_t,0.35,size_w,size_h)
                blobs_t_f = range_check(bin_img,100,crossover_point,point_t,0.9,size_w,size_h)
                blobs_b_c = range_check(bin_img,100,crossover_point,point_b,0.35,size_w,size_h)
                blobs_b_f = range_check(bin_img,100,crossover_point,point_b,0.9,size_w,size_h)
                ...
                if count3 >= 3:
                    ...
                    if last_list[0] != 999:
                        ...
                        if abs(tmp_theta - extra_list[0]) <= 50:
                            true_lines.append(the_line)
                count3 = 0
